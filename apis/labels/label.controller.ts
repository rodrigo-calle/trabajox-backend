...
import { createLabel, deleteLabel, getAllLabels, getLabelById, updateLabel } from "./label.service"
...
export const getLabelByIdHandler = async (req: Request, res: Response) => {
    try {
        const label = await getLabelById(parseInt(req.params.id))
...
export const updateLabelHandler = async (req: Request, res: Response) => {
    try {
        const label = await updateLabel(parseInt(req.params.id), req.body)
        res.status(200).json(label)
    } catch (error) {
        res.status(500).send({error})
    }
...
export const deleteLabelHandler = async (req: Request, res: Response) => {
    try {
        const id = await deleteLabel(parseInt(req.params.id))
