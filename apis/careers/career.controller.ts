...
import { createCareer, deleteCareer, getAllCareers, getCareerById, updateCareer } from "./career.service"
...
export const getCareerByIdHandler = async (req: Request, res: Response) => {
    try {
        const career = await getCareerById(parseInt(req.params.id))
...
export const updateCareerHandler = async (req: Request, res: Response) => {
    try {
        const career = await updateCareer(parseInt(req.params.id), req.body)
        res.status(200).json(career)
    } catch (error) {
        res.status(500).send({error})
    }
...
export const deleteCareerHandler = async (req: Request, res: Response) => {
    try {
        const id = await deleteCareer(parseInt(req.params.id))
